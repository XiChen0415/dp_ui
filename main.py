...
class DataValidate(tk.Frame):
    ...
    def Det_validate(self):
        #mbox.showinfo(title="待补全", message="功能待补全")
        filepath = '/Users/xiaoge/Downloads/Fiture_dataset/Detection.json'
        ...
        for e in annot['images']:
            if e['file_name'][0:3] == compare:
                pos.append(count)
            count += 1
        #imgpath = '/Users/xiaoge/Downloads/Fiture_dataset/2020-Jan-2/'+annot['images'][i]['file_name']
        count_ant = 0
        for e in annot['annotations']:
            if count_ant not in pos:
                e['category_id'] = 2
            if e['id'] == 1:
                count_ant += 1
        print(count_ant)
        ...
        for e in pos:
            cnt += 1
            if cnt == 10:
                path = annot['images'][e]['file_name']
                window_name = annot['images'][e]['file_name']
                img = cv2.imread('/Users/xiaoge/Downloads/Fiture_dataset/2020-Jan-2/'+ path)
                img_id = annot['images'][e]['id']
                cnt += 1
                for annt in annot['annotations']:
                    if (annt["image_id"] == annot['images'][e]['id']) and (annt['category_id'] != 2):
                        p1_x = annt['bbox'][0]
                        p1_y = annt['bbox'][1] 
    ...
    '''    
        ...
        tk.Bottom(self.DataValidate, image=imgtk).pack()
    '''
    def Pose_validate(self):
        ...
        for e in annot['annolist']:
            cnt +=1
            if cnt == 100: 
                path = str(e['imgidx'])+'.jpg'
                name = path 
                img = cv2.imread('/Users/xiaoge/Downloads/Fiture_dataset/2020-Jan-1/'+path)
